...
def fn_find_month(selected_value):
    pass
def fn_find_count():
    txt_stdID_find = std_id_find_textbox.value
    dict = {}
    if (len(txt_stdID_find) != 10):
        frm1.info(" اشتباه شماره دانش آموز", txt_stdID_find)
    else:
        d1 = DBApiuse({""}, "std_id", "parent_phone")
        dict_db_data = d1.get_data(txt_stdID_find)
        print(dict_db_data)
        report_run.create_report_data(dict_db_data)
        Text(options_box, text= txt_stdID_find + " : " + str(dict_db_data["count"][0]),font = 'B Titr', size = '10')
...
def month_choose(selected_value):
    global str_temp_SMS
    str_temp_SMS += ' ' + selected_value

def day_choose(selected_value):
    global str_temp_SMS
    str_temp_SMS = "هنرستان جابر:با سلام.فرزند شما در روز"
    str_temp_SMS += ' ' + selected_value

def absent_type_choose(selected_value):
    global str_temp_SMS
    str_temp_SMS += ' در ' + selected_value

def fn_build_message():
    global str_temp_SMS
    global str_sms
    str_temp_SMS += ' غیبت داشته است. '
    frm1.info("info", str_temp_SMS)
    str_sms = str_temp_SMS
    str_temp_SMS = ""

def fn_build_custom_message():
    global str_temp_SMS
    global str_sms
    str_temp_SMS = custom_message_textbox.value
    frm1.info("info", str_temp_SMS)
    str_sms = str_temp_SMS
    str_temp_SMS = ""
...
combo_day = Combo(frm1, options = list_day, command = day_choose, width = '8')

combo_month = Combo(frm1, options= month_list,\
                    command = month_choose, width = '8')

list_absent_type = ["کلاس مجازی","کلاس","امتحان"]
combo_absent_type = Combo(frm1, options = list_absent_type, command = absent_type_choose, width = '8')
Text(frm1, text="_____________")
